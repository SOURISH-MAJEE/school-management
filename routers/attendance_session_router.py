...
def get_sessions_by_class(class_id: int, db: Session = Depends(get_db)):
    return attendance_session_query.get_sessions_by_class(db, class_id)

@router.put("/{session_id}", response_model=AttendanceSessionResponse)
def update_session(session_id: int, session: AttendanceSessionUpdate,
                   db: Session = Depends(get_db)):
